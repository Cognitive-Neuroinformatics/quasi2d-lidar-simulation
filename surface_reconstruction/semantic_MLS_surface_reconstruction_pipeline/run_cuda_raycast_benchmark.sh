...
if [[ -z "$DATASET_ROOT" || -z "$CASEID" ]]; then
  echo "Required: --dataset-root PATH --caseid CASE" >&2
  exit 2
fi
if [[ -z "$RECON_ROOT" ]]; then
  RECON_ROOT="$DATASET_ROOT/semantic_aware_mls/semantic_static_mls/$CASEID"
fi
if [[ -z "$OUTPUT_ROOT" ]]; then
  OUTPUT_ROOT="$RECON_ROOT/scala2_raycast_cuda_${PRECISION}"
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
mkdir -p "$OUTPUT_ROOT"
LOG_ROOT="$OUTPUT_ROOT/benchmark_logs"
mkdir -p "$LOG_ROOT"
...
for d in "${DEV_ARR[@]}"; do GPU_ARGS+=("$d"); done
SENSOR_ARGS=()
read -ra SENSOR_ARR <<< "$SENSORS"
for s in "${SENSOR_ARR[@]}"; do SENSOR_ARGS+=("$s"); done

END_ARGS=()
if [[ -n "$END_FRAME" ]]; then END_ARGS=(--end-frame "$END_FRAME"); fi
OVERWRITE_ARGS=()
...
cleanup() {
  if [[ -n "${NVIDIA_PID:-}" ]]; then kill "$NVIDIA_PID" 2>/dev/null || true; fi
}
trap cleanup EXIT INT TERM

CMD=(python "$SCRIPT_DIR/raycaster/raycast_mls_scala2_cuda.py"
  ...
  --patch-radius 0.03
  --hit-radius 0.03
  --point-batch-size "$BATCH"
  --static-tile-cache 64
  --property-cache 64
  --gpu-cache-gb "$GPU_CACHE_GB"
  --devices "${GPU_ARGS[@]}"
  --precision "$PRECISION"
  --npz-compression "$NPZ_MODE"
  "${OVERWRITE_ARGS[@]}")
