#!/usr/bin/env python3
"""Run one complete strict-baseline scene with selectable CPU or CUDA raycasting.

All scientific/default execution settings are loaded from one JSON config file.
Command-line arguments are retained for scene selection, run control, and deliberate
overrides. Densification, PCL MLS, and both rasterizer implementations remain
unchanged; this file only builds and dispatches their commands.
"""
...
def require_dict(config: dict, key: str) -> dict:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"Config section '{key}' is missing or is not an object")
    return value
...
def parse_args(project: Path):
    # First pass: find the config before defining config-backed defaults.
    ...
    run_cfg = require_dict(config, "run")
    ray_cfg = require_dict(config, "raycast")
    shared_ray_cfg = require_dict(ray_cfg, "shared")
    cuda_cfg = require_dict(ray_cfg, "cuda")
    ...
    ap.add_argument("--rasterizer", choices=["cpu", "cuda"], default=run_cfg["rasterizer"], help="Final SCALA2 rasterizer backend. Densification and PCL MLS remain unchanged CPU baselines.")
    ap.add_argument("--clean", action="store_true", help="Delete this pipeline's shared reconstruction root (including previous CPU/CUDA raycast outputs) before the run")
    ...
    ap.add_argument("--fill-spacing-mode", choices=["fixed", "local_ring_density", "along_ring_density"], default=run_cfg["fill_spacing_mode"])
    ...
    ap.add_argument("--shm-min-free-gb", type=float, default=float(run_cfg["shm_min_free_gb"]), help="Minimum free /dev/shm space required for automatic RAM-disk PCL work files")
    ...
    ap.add_argument("--point-batch-size", type=int, default=None, help="Override backend point batch size from config for this run only")
    ...
    ap.add_argument("--cuda-devices", nargs="+", default=[str(v) for v in cuda_cfg["devices"]], help="CUDA device indices, e.g. --cuda-devices 0 1 2")
    ap.add_argument("--cuda-precision", choices=["float32", "float64"], default=cuda_cfg["precision"], help="CUDA precision from config unless explicitly overridden")
    ap.add_argument("--cuda-gpu-cache-gb", type=float, default=float(cuda_cfg["gpu_cache_gb_per_gpu"]), help="Per-GPU resident geometry cache budget used only by CUDA rasterization")
    ap.add_argument("--cuda-detailed-stats", action="store_true", default=bool(cuda_cfg.get("detailed_stats", False)), help="Enable the CUDA renderer's optional extra diagnostics")
    args = ap.parse_args()
...
def main():
    ...
    args, config = parse_args(project)
    run_cfg = require_dict(config, "run")
    densify_cfg = require_dict(config, "densification")
    mls_cfg = require_dict(config, "mls")
    ray_cfg = require_dict(config, "raycast")
    shared_ray_cfg = require_dict(ray_cfg, "shared")
    cpu_cfg = require_dict(ray_cfg, "cpu")
    cuda_cfg = require_dict(ray_cfg, "cuda")
    env_cfg = require_dict(config, "environment")

    raycast_point_batch_size = args.point_batch_size
    if raycast_point_batch_size is None:
        raycast_point_batch_size = int(cpu_cfg["point_batch_size"] if args.rasterizer == "cpu" else cuda_cfg["point_batch_size"])
    ...
    log_dir = dataset / "semantic_aware_mls" / "pipeline_benchmarks" / case / f"{run_id}_{args.rasterizer}"
    log_dir.mkdir(parents=True, exist_ok=True)

    if args.clean and reconstruction_root.exists():
        print(f"Removing previous optimized reconstruction: {reconstruction_root}")
        shutil.rmtree(reconstruction_root)

    # Record the effective values after command-line overrides. The original config is
    # kept untouched; this snapshot makes every run reproducible.
    resolved_config = copy.deepcopy(config)
    resolved_config["run"].update({
        "dataset_root": str(dataset), "caseid": case, "rasterizer": args.rasterizer,
        "npz_compression": args.npz_compression, "fill_spacing_mode": args.fill_spacing_mode,
        "pcl_threads": args.pcl_threads, "mls_workers": args.mls_workers,
        "attribute_workers": args.attribute_workers, "shm_min_free_gb": args.shm_min_free_gb,
        "raycast_sensors": list(args.raycast_sensors), "sensor_workers": args.sensor_workers,
        "raycast_start_frame": args.raycast_start_frame, "raycast_end_frame": args.raycast_end_frame,
    })
    resolved_config["raycast"]["shared"].update({
        "intersection_mode": args.intersection_mode, "patch_radius_m": args.patch_radius,
        "hit_radius_m": args.hit_radius, "minimum_range_m": args.minimum_range,
        "maximum_range_m": args.maximum_range, "first_mirror_side": args.first_mirror_side,
    })
    resolved_config["raycast"][args.rasterizer]["point_batch_size"] = int(raycast_point_batch_size)
    resolved_config["raycast"]["cuda"].update({
        "precision": args.cuda_precision, "gpu_cache_gb_per_gpu": args.cuda_gpu_cache_gb,
        "devices": [str(v) for v in args.cuda_devices], "detailed_stats": bool(args.cuda_detailed_stats),
    })
    resolved_config_path = log_dir / "resolved_pipeline_config.json"
    ...
    summary = {
        ...
        "settings": vars(args) | {
            ...
            "selected_raycast_point_batch_size": int(raycast_point_batch_size),
        },
        "paths": {
            "strict_npz": str(strict_npz), "support_npz": str(support_npz),
            "densified_npz": str(densified_npz), "reconstruction_root": str(reconstruction_root),
            "raycast_root": str(raycast_root), "log_dir": str(log_dir),
        },
        "stages": {},
    }
    ...
    if args.rasterizer == "cuda" and not args.skip_raycast:
        subprocess.run([args.python, str(project / "check_cuda_environment.py")], env=common_env, check=True)
    ...
    if not args.skip_densify:
        ...
        command = [
            ...
            "-s", str(dataset), "--caseid", case, "--strict_path", str(strict_npz), "-o", str(support_npz),
        ]
        if densify_cfg["generic_coverage"]:
            command.append("--generic_coverage")
        command.extend([
            "--analysis_voxel", str(densify_cfg["analysis_voxel_m"]),
            "--fill_spacing", str(densify_cfg["fill_spacing_m"]),
            "--fill_spacing_mode", args.fill_spacing_mode,
            "--adaptive_density_voxel", str(densify_cfg["adaptive_density_voxel_m"]),
            "--adaptive_density_neighbors", str(densify_cfg["adaptive_density_neighbors"]),
            "--adaptive_query_neighbors", str(densify_cfg["adaptive_query_neighbors"]),
            "--adaptive_fill_scale", str(densify_cfg["adaptive_fill_scale"]),
            "--adaptive_fill_min", str(densify_cfg["adaptive_fill_min_m"]),
            "--adaptive_fill_max", str(densify_cfg["adaptive_fill_max_m"]),
            "--min_separation", str(densify_cfg["min_separation_m"]),
        ])
        if densify_cfg["deduplicate"]:
            command.append("--deduplicate")
        command.extend([
            "--dedup_spacing", str(densify_cfg["dedup_spacing_m"]),
            "--max_candidates_per_family", str(densify_cfg["max_candidates_per_family"]),
            "--max_support", str(densify_cfg["max_support"]),
            "--densified_npz", str(densified_npz), "--npz_compression", args.npz_compression,
        ...
        env = common_env.copy()
        env.update({
            "OMP_NUM_THREADS": str(env_cfg["densify_omp_threads"]),
            "OPENBLAS_NUM_THREADS": str(env_cfg["densify_openblas_threads"]),
            "MKL_NUM_THREADS": str(env_cfg["densify_mkl_threads"]),
            "NUMEXPR_NUM_THREADS": str(env_cfg["densify_numexpr_threads"]),
        })
    ...
    if not args.skip_reconstruct:
        command = [
            ...
            "--dataset-root", str(dataset), "--caseid", case,
            "--static-input", str(densified_npz), "--dynamic-input-root", str(dynamic_root),
            "--pcl-executable", str(project_path(project, mls_cfg["pcl_executable"])),
            "--config", str(project_path(project, mls_cfg["config_file"])),
            "--output-root", str(reconstruction_root), "--stages", *[str(v) for v in mls_cfg["stages"]],
            "--tile-size", str(mls_cfg["tile_size_m"]), "--tile-halo", str(mls_cfg["tile_halo_m"]),
            "--pcl-threads", str(args.pcl_threads), "--mls-workers", str(args.mls_workers),
            "--attribute-workers", str(args.attribute_workers),
            "--minimum-label-confidence", str(mls_cfg["minimum_label_confidence"]),
        ]
        if not mls_cfg["exclude_points_in_tracked_boxes"]:
            command.append("--no-exclude-points-in-tracked-boxes")
        if mls_cfg["require_no_voxel_downsampling"]:
            command.append("--require-no-voxel-downsampling")
        command.extend(["--npz-compression", args.npz_compression])
        if mls_cfg["overwrite"]:
        ...
        env.update({
            ...
            "OPENBLAS_NUM_THREADS": str(env_cfg["reconstruct_openblas_threads"]),
            "MKL_NUM_THREADS": str(env_cfg["reconstruct_mkl_threads"]),
            "NUMEXPR_NUM_THREADS": str(env_cfg["reconstruct_numexpr_threads"]),
        })
    ...
    if not args.skip_raycast:
        common_raycast = [
            "--dataset-root", str(dataset), "--caseid", case,
            "--reconstruction-root", str(reconstruction_root), "--output-root", str(raycast_root),
            "--sensors", *args.raycast_sensors, "--start-frame", str(args.raycast_start_frame),
            "--first-mirror-side", str(args.first_mirror_side), "--minimum-range", str(args.minimum_range),
            "--max-range", str(args.maximum_range), "--intersection-mode", args.intersection_mode,
            "--patch-radius", str(args.patch_radius), "--hit-radius", str(args.hit_radius),
            "--point-batch-size", str(raycast_point_batch_size),
            "--static-tile-cache", str(shared_ray_cfg["static_tile_cache"]),
            "--property-cache", str(shared_ray_cfg["property_cache"]),
            "--npz-compression", args.npz_compression,
        ]
        if args.rasterizer == "cpu":
            command = [args.python, str(project / "raycaster" / "raycast_mls_scala2.py"), *common_raycast,
                "--ray-neighbor-count", str(cpu_cfg["ray_neighbor_count"]),
                "--candidate-lookup", str(cpu_cfg["candidate_lookup"]),
                "--sensor-workers", str(args.sensor_workers),
                "--ckdtree-workers", str(cpu_cfg["ckdtree_workers"])]
        else:
            command = [args.python, str(project / "raycaster" / "raycast_mls_scala2_cuda.py"), *common_raycast,
                "--gpu-cache-gb", str(args.cuda_gpu_cache_gb), "--devices", *args.cuda_devices,
                "--precision", args.cuda_precision]
            if args.cuda_detailed_stats:
                command.append("--detailed-stats")
        if shared_ray_cfg["overwrite"]:
            command.append("--overwrite")
        if args.raycast_end_frame is not None:
            command.extend(["--end-frame", str(args.raycast_end_frame)])
        env = common_env.copy()
        env.update({
            "OMP_NUM_THREADS": str(env_cfg["raycast_omp_threads"]),
            "OPENBLAS_NUM_THREADS": str(env_cfg["raycast_openblas_threads"]),
            "MKL_NUM_THREADS": str(env_cfg["raycast_mkl_threads"]),
            "NUMEXPR_NUM_THREADS": str(env_cfg["raycast_numexpr_threads"]),
        })
        summary["stages"]["raycast"] = run_stage("raycast", command, env, log_dir)
        summary["stages"]["raycast"]["detail"] = summarize_raycast(raycast_root / "raycast_summary.json")
        raycast_root.mkdir(parents=True, exist_ok=True)
        shutil.copy2(resolved_config_path, raycast_root / "resolved_pipeline_config.json")
    ...
    summary_path = log_dir / "unified_pipeline_benchmark.json"
    with summary_path.open("w") as stream:
        json.dump(summary, stream, indent=2, default=str)

    print("\n" + "=" * 80)
    print(f"UNIFIED PIPELINE COMPLETE — {args.rasterizer.upper()} RASTERIZER")
    print("=" * 80)
    ...
    if isinstance(densify_detail, dict):
        ranked = sorted(((name, float(value)) for name, value in densify_detail.items() if name.endswith("_s") and isinstance(value, (int, float)) and name != "total_s"), key=lambda item: item[1], reverse=True)
        if ranked:
    ...
    if isinstance(reconstruction_detail, dict):
        print("\nMLS stage timings:")
        for name, seconds in sorted((reconstruction_detail.get("stage_timings_seconds") or {}).items(), key=lambda item: float(item[1]), reverse=True):
    ...
    if isinstance(raycast_detail, dict) and raycast_detail.get("completed_sensor_frames", 0):
        ...
        for name, seconds in sorted(raycast_detail.get("timing_breakdown_mean_s", {}).items(), key=lambda item: float(item[1]), reverse=True):
            print(f"  {name:34s} {float(seconds):10.2f} s")

    print(f"Config        : {args.config}")
    print(f"Resolved config: {resolved_config_path}")
    print(f"Rasterizer    : {args.rasterizer}")
    print(f"Raycast output: {raycast_root}")
