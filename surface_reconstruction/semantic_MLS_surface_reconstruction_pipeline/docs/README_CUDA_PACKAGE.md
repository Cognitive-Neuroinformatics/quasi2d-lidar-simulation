# Semantic static MLS— CPU-optimized reference before CUDA
...
The expensive parts are the large cKDTree queries used by generic coverage, plane fitting and metadata-source mapping, plus very large NPZ writes. The original implementation also repeatedly scanned all points by ring sector and used Python loops. (Maybe cpp shift?? TODO!)
...
- vectorized neighbouring-sector Z evaluation (# what about slope transition?)
...
- densified PCD is optional because MLS consumes the NPZ, not the PCD (do i even keep pcd because LiDARGS kept it??)
...
- default full-pipeline split for the 24 (PC) or 32 (server)logical-CPU workstation: **3 MLS workers × 8/10 PCL threads**
...
- background semantic family is classified once per tile rather than repeatedly rescanned for every group ### TODO!! SEMANTIC ASSOCIATION SEEMS TO BE WRONG
...
- 3D tile-FOV culling before geometry processing
...
```bash
cd /home/samanti/Documents/Uni_Bremen/PhD/my_workspace/lidar_surface_reconstruction/semantic_MLS_surface_reconstruction_pipeline

./run_one_scene_cpu_optimized.sh \
  --clean \
  --dataset-root /data/waymo/road_reconstruction_study \
...
This package also contains the CUDA SCALA2 renderer. See [`CUDA_RAYCAST_README.md`](CUDA_RAYCAST_README.md).